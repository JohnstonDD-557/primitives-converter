...
class BigWorldModelLoader:
    ...
    def load_from_file(self, model_filepath, import_empty, debug_mode, displacement, rotation, scale, load_bones, NewVisual_mode = False,import_models = True):
        ...
        if visual_filepath and primitives_filepath: #If visual exists
            ...
            if visual.find('renderSet') is not None: #Check if renderSet node(s) exist in the visual file
                for renderSet in visual.findall('renderSet'): #For each renderSet
                    ...
                    if import_models: #If importing models
                        ...
                        meshes.append(mesh_name) # 添加与骨骼绑定的网格名称

                        bmesh = bpy.data.meshes.new(mesh_name) #Create new blender object with name from visual

                        dataMesh = LoadDataMesh(primitives_filepath, vres_name, pres_name, debug_mode, load_bones) #Compiles the vertices and indices into lists
                        ...
                        if 'true' in renderSet.find('treatAsWorldSpaceObject').text: #If has vector groups
                            if dataMesh.bones_info: #If bones exist
                                ...
                                for node in renderSet.findall('node'):
                                    ...
                                    empties.append(bone_name) # 添加骨骼名称到列表
                    ...
                    if load_bones:
                        if dataMesh.bones_info: #If bones exist
                            armature = auto_convert_empties_to_bones(empties, meshes, mesh_name + '_Armature') # 自动将空物体转换为骨骼系统
